...
def lambda_handler(event, context):
  try:
    ...
    repo=event["Repo"]
    region=event["Region"]
    tagkey01=event["TargetDBTag"]["SERVER_ENV"]
    tagvalue01=event["TargetDBTag"]["SERVER_ENV_VALUE"]
    tagkey02=event["TargetDBTag"]["SERVER_NAME"]
    tagvalu02=event["TargetDBTag"]["SERVER_NAME_VALUE"]
    src_db=event["Source"]
    tgt_db=event["Target"]
   ...
   # if reservations['ResponseMetadata']['HTTPStatusCode'] == 200:
    ...
    for reservation in reservations:
        for instance in reservation["Instances"]:
             ...
             Parameters={
               "commands": ["runuser -l  oracle -c 'cd clone_auto; sh dbctl-erp -source_db "+src_db+" -target_db "+tgt_db+" -action TARGET_PREP -phase START_DB'"]
            },  # Command to Start EBS DB Service
        ) 
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] != 200:
           raise Exception ("Issue in Starting Database Servic")
    ...
    statusCode=201
    print('***Error - Failed to run ssm send command to stop database services.')
    print(type(e), ':', e)
    return {
              'StatusCode': statusCode,
              'CommandId' : "NULL",
